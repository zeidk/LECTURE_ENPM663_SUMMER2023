...
\begin{center}
\emph{A \CC Node needs to be compiled while a Python Node does not.}
\begin{compactitem}
\footnotesize
...
{\large \mytodo Write a Python Node } \doublerulefill
...
\small
    ...
    {\footnotesize Node} & \myNode{first_node_py} \\
    \end{tabular} 
...
\item Run the Node: \myTerminal{ros2 run first_package_py first_node_py}
\begin{compactitem}
    \item[] \mywarning The argument \myTerminalBlank{first_node_py} is from the name  provided in \myFile{setup.py}
\end{compactitem}
\item Inspect the Node: \myTerminal{ros2 node info /first_node_py}
\item \mytodo Create another script \myFile{second_node.py} in the same package as an exercise.
\end{compactitem}
